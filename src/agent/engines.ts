...
function normalizeToUsdPerMt(price: number | null, currency: string | null, unit: string | null): number | null {
  ...
  if (u === 'bag') return usd * 20;
  if (u === 'litre' || u === 'liter') return usd * 1000;
  return null;
}
...
function buildOperationalIntelligence(input: EngineInput) {
  ...
  const nextEta =
    shipments.map(s => s.expected_arrival).filter(Boolean).sort()[0] ?? null;

  const inTransitQty = shipments
    .filter(s => /transit/i.test(s.status))
    .reduce((n, s) => n + (Number(s.quantity ?? 0) || 0), 0);
...
export function pricePointEngine(input: EngineInput): PricePoint[] {
  ...
  for (const row of input.marketRows) {
    ...
    const loc =
      [row.city, row.market, row.country].filter(Boolean).join(', ') ||
      row.country;
  ...
  points.sort((a, b) => {
    ...
    return (b.observation_date ?? '').localeCompare(
      a.observation_date ?? ''
    );
  });
...
function classifySupplyFromText(text: string): SupplyLevel | null {
  ...
  if (/\b(severe shortage|critical shortage|acute shortage|supply crisis|humanitarian aid|emergency supplies|famine|starvation)\b/.test(t)) {
    return 'Critical';
  }

  if (/\b(shortage|tight supply|supply disruption|low stocks?|depleted|scarce|constrained|insufficient supply|supply shortfall|crop failure|failed harvest|poor harvest|reduced harvest|lower production|declining production|production decline|export ban|export restriction)\b/.test(t)) {
    return 'Tight';
  }

  if (/\b(abundant|surplus|oversupply|bumper harvest|record (?:production|harvest|crop)|high production|excess supply|glut|stocks? (?:high|rising|ample)|ample supplies|overproduction|increased production|rising production|higher production|good harvest|strong harvest)\b/.test(t)) {
    return 'High';
  }

  if (/\b(adequate suppl(?:y|ies)|supplies? (?:remained?|are|were) (?:adequate|sufficient|normal|stable)|normal supply|stable supply|sufficient supply|steady supply|ample|available|in stock|stocks? (?:normal|stable|adequate)|harvest (?:normal|on track|progressing))\b/.test(t)) {
    return 'Normal';
  }
...
function classifyDemandFromText(text: string): DemandLevel | null {
  ...
  if (/\b(surg(?:e|ing)|soaring|explosive demand|skyrocket|surge in demand|sharp increase in demand|spike in demand)\b/.test(t)) {
    return 'Surging';
  }

  if (/\b(demand (?:increased|increasing|rising|grew|strong|robust|high)|strong demand|robust demand|high demand|rising consumption|increased consumption|growing consumption|buying activity (?:increased|strong|high)|import demand (?:increased|rising|strong)|household demand (?:increased|strong)|industrial demand (?:increased|strong|rising)|higher consumption|strong (?:purchases|buying))\b/.test(t)) {
    return 'Strong';
  }

  if (/\b(demand (?:decreased|declining|falling|weak|low|dropped)|weak demand|low demand|sluggish demand|soft demand|falling consumption|declining consumption|reduced consumption|lower demand|weak (?:purchases|buying)|decreased consumption)\b/.test(t)) {
    return 'Weak';
  }

  if (/\b(normal demand|stable demand|steady demand|moderate demand|demand (?:stable|steady|normal|remained? stable)|demand (?:remains?|is) (?:strong|steady|stable))\b/.test(t)) {
    return 'Normal';
  }

  if (/demand\b[^.]{0,40}\b(?:strong|robust|high|rising|increasing|growing)\b/.test(t)) {
    return 'Strong';
  }

  if (/demand\b[^.]{0,40}\b(?:weak|low|declining|falling|decreasing)\b/.test(t)) {
    return 'Weak';
  }
...
export function supplyDemandEngine(input: EngineInput): {
  supply: SupplyLevel;
  demand: DemandLevel;
  evidence: string;
  conflicts: string[];
  ...
  const { supplySignals, demandSignals } =
  ...
  return {
    supply,
    demand,
    evidence: evidence.join('; ') || INSUFFICIENT,
    conflicts,
  };
}
...
function classifyDemandTrendFromText(
  ...
  if (/\b(increas|rising|growing|surging|soaring|grew|boost|expanding|climbing)\b/.test(t)) {
    return 'UP';
  }

  if (/\b(decreas|declin|falling|dropping|weaker|lower|slowing|shrinking|contracting|reduced)\b/.test(t)) {
    return 'DOWN';
  }

  if (/\b(stable|steady|flat|unchanged|moderate)\b/.test(t)) {
    return 'FLAT';
  }
  ...
  if (observed.length > 0) {
    ...
    if (levels.length > 1) {
      ...
      const trends = inferred
        .map((s) => s.trend)
        .filter((tr) => tr !== 'UNKNOWN');

      if (trends.length > 0) {
        const uniqueTrends = [...new Set(trends)];
        trend =
          uniqueTrends.length === 1 ? uniqueTrends[0] : 'UNKNOWN';
      }
    }
  }
...
export function sentimentEngine(input: EngineInput): {
  ...
  if (supply === 'High') {
    signal += 1;
    reasons.push('Supply is high (price-easing)');
  }

  if (supply === 'Tight') {
    signal -= 1;
    reasons.push('Supply is tight (price-supportive)');
  }

  if (supply === 'Critical') {
    signal -= 2;
    reasons.push('Supply is critical (price-positive)');
  }
  ...
  if (points.length >= 2) {
    ...
    if (recent != null && prior != null) {
      ...
      if (change > 0.03) {
        signal += 1;
        reasons.push(`Prices up ${(change * 100).toFixed(1)}% recently`);
      } else if (change < -0.03) {
        signal -= 1;
        reasons.push(`Prices down ${(-change * 100).toFixed(1)}% recently`);
      }
    }
  }
...
export function landedCostEngine(
  ...
  const originRows = input.marketRows.filter(
    (r) =>
      ...
      (
        r.origin?.toLowerCase() === input.origin!.toLowerCase() ||
        r.country?.toLowerCase() === input.origin!.toLowerCase()
      )
  );
  ...
  const liveFxRate = input.fxRates.find(
    (f) =>
      f.base_currency === purchase.currency?.toUpperCase() &&
  ...
  const usdRate =
    ...
    (purchase.currency === 'USD'
      ? 1
      : FALLBACK_USD_RATES[purchase.currency!.toUpperCase()]);

  const purchaseUsd = usdRate
    ? purchase.price * usdRate
    : null;

  const freight =
    purchaseUsd != null ? purchaseUsd * 0.12 : null;

  const transit =
    purchaseUsd != null ? purchaseUsd * 0.03 : null;

  const customs =
    purchaseUsd != null ? purchaseUsd * 0.05 : null;

  const taxes =
    purchaseUsd != null ? purchaseUsd * 0.02 : null;
  ...
  const components: LandedCostComponent[] = [
    {
      ...
      value: purchaseUsd,
      currency: 'USD',
      source: purchase.source ?? 'stored',
      data_status: purchase.data_status,
      confidence: purchase.confidence,
    },
    {
      label: 'Freight (ESTIMATED BY MODEL)',
      value: freight,
      currency: 'USD',
    ...
    {
      label: 'Transit & Handling (ESTIMATED BY MODEL)',
      value: transit,
      currency: 'USD',
      source: 'model-estimate',
      data_status: 'ESTIMATED',
    ...
    {
      label: 'Customs (ESTIMATED BY MODEL)',
      value: customs,
      currency: 'USD',
      source: 'model-estimate',
      data_status: 'ESTIMATED',
    ...
    {
      label: 'Taxes (ESTIMATED BY MODEL)',
      value: taxes,
      currency: 'USD',
      source: 'model-estimate',
      data_status: 'ESTIMATED',
    ...
    {
      label: 'Handling & Storage (ESTIMATED BY MODEL)',
      value: handling,
      currency: 'USD',
      source: 'model-estimate',
      data_status: 'ESTIMATED',
  ...
  const perUnit =
    purchaseUsd != null
      ? (total / purchaseUsd) * purchase.price!
      : null;
  ...
  return {
    ...
    note:
      'Cost components beyond purchase price are ESTIMATED BY MODEL based on typical trade-cost ratios. Replace with verified freight, customs, and tax quotes when available.' +
      (usingFallbackFx
...
export function anomalyEngine(input: EngineInput): Anomaly[] {
    ...
    if (recent != null && prior != null) {
      ...
      if (change > 0.1) {
        anomalies.push({
          ...
          description: `Price up ${(change * 100).toFixed(1)}% vs prior observation.`,
        });
      }
      ...
      if (change < -0.1) {
        anomalies.push({
          ...
          severity: change < -0.25 ? 'HIGH' : 'MEDIUM',
          description: `Price down ${(-change * 100).toFixed(1)}% vs prior observation.`,
        });
      }
    }
  }
  ...
  if (supply === 'Critical') {
    anomalies.push({
      ...
      severity: 'CRITICAL',
      description: 'Supply classified as Critical.',
    });
  }
  ...
  if (supply === 'Tight') {
    anomalies.push({
      type: 'Tight supply',
      severity: 'MEDIUM',
      description: 'Supply classified as Tight.',
    });
  }
  ...
  if (demand === 'Surging') {
    anomalies.push({
      type: 'Demand surge',
      severity: 'HIGH',
      description: 'Demand classified as Surging.',
    });
  }
...
export function forecastEngine(
  ...
  if (points.length >= 2) {
    ...
    if (recent != null && prior != null) {
      ...
      if (change > 0.03) {
        priceDir = 'UP';
        rationale.push(
          `Recent prices up ${(change * 100).toFixed(1)}%.`
        );
      } else if (change < -0.03) {
        priceDir = 'DOWN';
        rationale.push(
          `Recent prices down ${(-change * 100).toFixed(1)}%.`
        );
      } else {
        priceDir = 'FLAT';
        rationale.push('Prices roughly flat recently.');
      }
  ...
  if (supply === 'Tight' || supply === 'Critical') {
    priceDir = priceDir === 'DOWN' ? 'FLAT' : 'UP';
    rationale.push('Tight supply pressures prices upward.');
  }

  if (demand === 'Strong' || demand === 'Surging') {
    priceDir = priceDir === 'DOWN' ? 'FLAT' : 'UP';
    rationale.push('Strong demand supports prices.');
  }

  if (supply === 'High') {
    rationale.push('High supply eases price pressure.');
  }

  const risk: Forecast['market_risk'] =
    sentiment === 'Negative' || supply === 'Critical'
      ? 'HIGH'
      : sentiment === 'Cautious' || supply === 'Tight'
      ? 'MEDIUM'
      : 'LOW';
  ...
  const supplyDir: Forecast['supply_direction'] =
    supply === 'High'
      ? 'UP'
      : supply === 'Tight' || supply === 'Critical'
      ? 'DOWN'
      : 'FLAT';

  const demandDir: Forecast['demand_direction'] =
    demand === 'Strong' || demand === 'Surging'
      ? 'UP'
      : demand === 'Weak'
      ? 'DOWN'
      : 'FLAT';
...
export function recommendationEngine(
  ...
  if (
    ...
    const destRows = input.marketRows.filter(
      (r) =>
    ...
    if (
      ...
      if (
        ...
        const margin =
          (destPerMt - landedPerMt) / landedPerMt;
...
export function sourceEngine(input: EngineInput): {
  sources: DataSource[];
  conflicts: string[];
} {
...
export function researchSummaryEngine(
  ...
  return top
    .map(
      (r, i) =>
        `${i + 1}. ${r.title}: ${r.snippet.slice(0, 180)}`
    )
    .join('\n');
}
...
export function buildFindings(
  ...
  const commodity =
    input.commodity ?? 'the requested commodity';
  ...
  } = recommendationEngine(input, landed);
  ...
  const execSummary =
    ...
    `${input.origin ? ` from ${input.origin}` : ''}` +
    `${input.destination ? ` to ${input.destination}` : ''}` +
    `${input.city ? ` in ${input.city}` : ''}. ` +
    (
      points.length > 0
        ? `${points.length} price observation(s) found.`
        : 'No stored price observations found.'
    ) +
    ` ${input.fxRates.length > 0 ? 'FX rates available.' : 'FX rates unavailable.'}` +
    ` ${sentiment !== 'Highly Uncertain' ? `Market sentiment: ${sentiment}.` : 'Market sentiment: HIGHLY UNCERTAIN.'}` +
    ` Operational position: ${operational.summary}.` +
    ` Recommendation: ${rec}.`;
  ...
  return {
    ...
    origin_market: input.origin
      ? (
          points
            ...
            .map(
              (p) =>
                `${p.label}: ${p.price ?? '—'} ${p.currency ?? ''}/${p.unit ?? ''}`
            )
            .join('; ') ||
          INSUFFICIENT
        )
    ...
    government_trade_updates:
      input.researchResults.find(
        (r) =>
          /custom|tariff|ban|export|import|policy|regulation/i.test(
            r.title + r.snippet
          )
      )?.snippet ??
      'No government/trade updates identified in available sources.',
...
export function evaluationEngine(
  ...
  // ------------------------------------------------------------
  // FIX:
  // Distinguish field-reported market stock from
  // operational/company warehouse stock.
  // ------------------------------------------------------------
  ...
  if (stockRaw) {
    supplyText +=
      ` Field stock availability: ${stockRaw}.`;

    if (/depleted|low/i.test(stockRaw)) {
      reasons.push(
        'Low/depleted stock reported.'
      );
    }
  }
  ...
  if (
    ...
    supplyText +=
      ` Operational stock: ${operational.stock.available.toLocaleString()} ${operational.stock.unit ?? ''} available, ${operational.stock.in_transit.toLocaleString()} ${operational.stock.unit ?? ''} in transit, ${operational.stock.expected_incoming.toLocaleString()} ${operational.stock.unit ?? ''} expected incoming.`;
  ...
  if (competitorRaw) {
    ...
    competitionText =
      'No competitor activity recorded — UNKNOWN.';
    warnings.push(
  ...
  if (points.length === 0) {
    ...
    if (destPrice?.price != null) {
      ...
      if (
        ...
        if (margin > 0.1) {
          priceText =
            `Attractive — estimated margin ~${(margin * 100).toFixed(0)}% above landed cost.`;

          reasons.push(
            `Price margin ~${(margin * 100).toFixed(0)}% — attractive.`
          );
        } else if (margin > 0) {
          priceText =
            `Thin margin (~${(margin * 100).toFixed(0)}%) — proceed with caution.`;
        } else {
          priceText =
            `Unattractive — landed cost exceeds destination price by ${(-margin * 100).toFixed(0)}%.`;
    ...
    priceText =
      `${points.length} price observation(s) available. No landed cost estimate for comparison.`;
  }
  ...
  if (criticalAnomalies.length > 0) {
    riskText =
      `ELEVATED — ${criticalAnomalies.length} critical/high anomaly/anomalies: ${criticalAnomalies.map((a) => a.type).join(', ')}.`;
